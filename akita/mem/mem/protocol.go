...
// WithInstPC sets the instruction PC for the request (for compatibility)
func (b ReadReqBuilder) WithInstPC(pc uint64) ReadReqBuilder {
	// Store PC in Info field for compatibility
	if b.info == nil {
		b.info = make(map[string]interface{})
	}
	if infoMap, ok := b.info.(map[string]interface{}); ok {
		infoMap["InstPC"] = pc
	}
	return b
}
