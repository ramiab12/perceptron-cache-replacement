...
# Comprehensive BiCG Test Script
# Tests BiCG with perceptron vs LRU cache replacement
# Measures cache performance and timing metrics from database

set -e

echo "🚀 BiCG Comprehensive Perceptron vs LRU Test"
echo "============================================"
echo ""
...
RESULTS_FILE="$RESULTS_DIR/bicg_comprehensive_results_$TIMESTAMP.txt"

# Matrix sizes (x=y)
SIZES=(256 272 288 304 320 336 352)
...
BiCG Comprehensive Perceptron vs LRU Test Results
=================================================
Test started: $(date)
Configuration:
- Sizes (x=y): ${SIZES[@]}
...
run_bicg_test() {
    ...
    cd "$repo_dir/amd/samples/bicg"
    rm -f akita_sim*.sqlite3

    go build -o "$binary_name" > /dev/null 2>&1 || { echo "BUILD_FAILED 0 0 0"; return; }

    local flags="-x $size -y $size -timing -report-cache-hit-rate -report-cache-latency"
...
test_size() {
    ...
    echo ""
    echo "🔬 Testing BiCG ${size}x${size}"
    echo "=============================="

    echo "🔄 Testing Perceptron..."
    read p_hits p_misses p_hit_rate p_total_time p_avg_latency <<<$(run_bicg_test "$size" "/home/rami/perceptron_research/mgpusim" "bicg_perc")
    echo "  ✅ Perceptron: hits=$p_hits, misses=$p_misses, hit-rate=$p_hit_rate%, total-time=${p_total_time}s, avg-latency=${p_avg_latency:-0}s"

    echo "🔄 Testing LRU..."
    read l_hits l_misses l_hit_rate l_total_time l_avg_latency <<<$(run_bicg_test "$size" "/home/rami/mgpusim_original" "bicg_lru")
...
echo "🧪 Starting BiCG comprehensive tests..."
echo ""
...
=====================================================================================================
Test completed: $(date)
Total tests run: $total_tests sizes
